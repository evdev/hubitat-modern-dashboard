...
function mockHtmlTilesCatalog() {
  return MOCK_HTML_TILES.map((tile) => ({ ...tile }));
}

function normalizeHtmlTileTitle(raw, fallback = "HTML") {
  let t = raw == null ? "" : String(raw).replace(/[\u0000-\u001F\u007F]/g, " ").trim();
  if (t.length > 80) t = t.slice(0, 80).trim();
  return t || String(fallback || "HTML").trim() || "HTML";
}
const MOCK_TRACKS = [
  "Daft Punk — Get Lucky",
...
function ensureEmbedConfig() {
  ...
  if (!state.config.htmlTitles || typeof state.config.htmlTitles !== "object") state.config.htmlTitles = {};
...
const server = createServer(async (req, res) => {
  ...
  if (p === "/settings/favorites-layout" && req.method === "POST") {
    ...
    if (body.htmlTitles != null && (typeof body.htmlTitles !== "object" || Array.isArray(body.htmlTitles))) {
      res.writeHead(400, { "Content-Type": "application/json" });
      return res.end('{"ok":false,"error":"invalid htmlTitles"}');
    }
    ...
    if (body.htmlTitles != null) {
      ...
      const activeHtmlIds = new Set(reconciled.map(htmlIdFromLayoutKey).filter(Boolean));
      state.config.htmlTitles = Object.fromEntries(
        Object.entries(state.config.htmlTitles || {}).filter(([id]) => activeHtmlIds.has(id))
      );
    }
    res.writeHead(200, { "Content-Type": "application/json" });
    return res.end(JSON.stringify({
