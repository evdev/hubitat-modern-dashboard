...
export function convertSarApp(appId, appData, appMeta, deviceMeta, lightIds, outletIds) {
  ...
  const at2T = String(settingValue(settings, "at2T") || "").trim().toLowerCase();
  const slots = ["primary"];
  // Non-empty at2T (time/sunrise/sunset) is the on/off cycle flag. Clock second
  // times do not appear as doAntiAction subscriptions in App Export (scheduled
  // jobs only). Leftover at2Time="00:00" / *2Offset with empty at2T is not a cycle.
  if (at2T === "time" || at2T === "sunrise" || at2T === "sunset") slots.push("secondary");
  ...
  for (const slot of slots) {
    ...
    const slotName = slot === "secondary" ? `${name} (${slotOnInfo.on ? "on" : "off"})` : name;
