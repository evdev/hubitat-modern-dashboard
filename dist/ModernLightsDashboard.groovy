...
@Field private static final String MLD_DEPLOYED_VERSION = "0.4.17"
...
def mainPage() {
    ...
    dynamicPage(name: "mainPage", uninstall: true, install: true) {
        section("Get started", hideable: true, hidden: introSectionCollapsed()) {
            ...
            paragraph "<small>Version 0.4.17 · Ephrayim (evdev) · Apache License 2.0 · <a href='https://github.com/evdev/hubitat-modern-dashboard' target='_blank'>Source</a></small>"
        }
        if (assetsOk) {
...
def renderIndex() {
    ...
    def iconHref = "https://raw.githubusercontent.com/evdev/hubitat-modern-dashboard/master/dist/upload/mld-icon-192-0.4.17.png"
...
def renderManifest() {
    ...
    for (def size : ["192", "512", "1024"]) {
        def src = "https://raw.githubusercontent.com/evdev/hubitat-modern-dashboard/master/dist/upload/mld-icon-${size}-0.4.17.png"
...
def setThermostatFanModeCmd(t, modeStr) {
    ...
    if (!(t.hasCapability("ThermostatFanMode") || t.hasAttribute("thermostatFanMode") || tstatHasComfortFanSpeed(t))) return false
    ...
    try {
        if (kind == "daily" || kind == "weekly") {
            ...
            if (atMs != null && atMs > nowMs) {
                runOnce(new Date(atMs), "scheduledJobHandler", scheduleJobOptions(id))
                result.registered++
                nextFire = atMs
            } else if (atMs != null && atMs <= nowMs) {
...
def runScheduleThermostatAction(action, result) {
    ...
    for (id in ids) {
        ...
        if (deviceFailed) result.failed << id.toString()
...
def schedulesSave() {
    ...
    def deviceErr = schedulesUnknownDeviceError(s)
    if (deviceErr) {
        return renderJsonNoStore( "{\"ok\":false,\"error\":${jsonStr(deviceErr)}}".toString(), 422)
    }
    def map = parsed.map instanceof Map ? parsed.map : [:]
    def priorJson = state.schedulesJson
    def id = s.id
