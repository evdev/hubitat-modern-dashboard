...
def mainPage() {
    ...
    dynamicPage(name: "mainPage", uninstall: true, install: true) {
        section("About Modern Dashboard", hideable: true, hidden: false) {
            ...
            paragraph "<small>Version 0.2.37 · Ephrayim (evdev) · Apache License 2.0 · <a href='https://github.com/evdev/hubitat-modern-dashboard' target='_blank'>Source</a></small>"
        }
        section("Devices") {
        ...
        section("Options") {
            ...
            input "showOfflineDevices", "bool", title: "Show inactive devices as offline (grayed out)", defaultValue: false, required: false
            paragraph "<small>Uses Hubitat device status (INACTIVE). Quiet but healthy devices may occasionally appear offline.</small>"
        }
        section("Scheduler") {
...
def renderData() {
    ...
    out << ",\"showOfflineDevices\":" << (showOfflineDevices == true)
    ...
    if (windowShades) {
        for (d in windowShades) {
            ...
            def shadeSt = safeCurrent(d, "windowShade")
            def shadePos = safeCurrent(d, "position")
            out << "{\"i\":" << d.id
            out << ",\"n\":" << jsonStr(d.displayName)
            out << ",\"r\":" << (rid == null ? "null" : rid.toString())
...
def renderDevice() {
    ...
    if (shade != null) {
        ...
        def shadeSt = safeCurrent(shade, "windowShade")
        def shadePos = safeCurrent(shade, "position")
        def out = new StringBuilder()
        out << "{\"i\":" << shade.id
        out << ",\"st\":" << jsonStr(shadeSt)
        out << ",\"pos\":" << numOrNull(shadePos)
        appendOfflineJson(out, shade)
        out << "}"
        return render(contentType: "application/json", data: withAuthJson(out.toString()), status: 200)
    }
...
def runShadeCmd(dev, c, v) {
    switch (c) {
        ...
        case "setPosition":
            ...
            break
        case "stop":
            if (!dev.hasCommand("stopPositionChange")) throw new IllegalArgumentException("unsupported command")
            dev.stopPositionChange()
            break
        default:
            throw new IllegalArgumentException("unknown command")
    }
