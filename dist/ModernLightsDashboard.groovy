...
@Field private static final String MLD_DEPLOYED_VERSION = "0.4.11"
...
def mainPage() {
    ...
    dynamicPage(name: "mainPage", uninstall: true, install: true) {
        section("Get started", hideable: true, hidden: introSectionCollapsed()) {
            ...
            paragraph "<small>Version 0.4.11 · Ephrayim (evdev) · Apache License 2.0 · <a href='https://github.com/evdev/hubitat-modern-dashboard' target='_blank'>Source</a></small>"
        }
        if (assetsOk) {
...
def renderIndex() {
    ...
    def iconHref = "https://raw.githubusercontent.com/evdev/hubitat-modern-dashboard/beta/dist/upload/mld-icon-192-0.4.11.png"
...
def renderManifest() {
    ...
    for (def size : ["192", "512", "1024"]) {
        def src = "https://raw.githubusercontent.com/evdev/hubitat-modern-dashboard/beta/dist/upload/mld-icon-${size}-0.4.11.png"
...
def lightCommandNextStep() {
    ...
    if (dev && step.cmds) {
        ...
        for (pair in step.cmds) {
            ...
            try {
                runLightCmd(dev, c, v)
                def kind = job.kind?.toString()
                def kindBit = kind ? " (${kind})" : ""
                logControl("manual", "${deviceLabel(dev)} ${cmdDetail(c, v)}${kindBit}")
            } catch (e) {
...
def setHubModeFromName(modeName) {
    ...
    try {
        ...
        logControl("manual", "hub mode → ${modeName}")
...
def alertsArmFromValue(armed) {
    ...
    try {
        logControl("manual", "${deviceLabel(sw)} ${wantOn ? 'on' : 'off'} (alerts arm)")
        if (wantOn) sw.on()
        else sw.off()
...
def activateSceneFromId(id) {
    ...
    try {
        def sceneName = scenesMap[sceneId]?.toString()
        logControl("manual", "scene ${sceneName ?: sceneId}")
...
def schedImportApplyOk(okRows) {
    ...
    def priorJson = null
    def count = 0
    def applyError = withSchedulesLock {
        def map = parseSchedulesMap()
        priorJson = state.schedulesJson
        try {
            okRows.each { row ->
    ...
    if (applyError) {
        ...
        return [error: applyError, count: 0]
    }
...
// A single `schedulerNextFireHandler` runOnce is armed for the earliest
// time-based schedule. When it fires, every due schedule is processed in one
// top-level execution, then the next dispatcher is armed. This avoids Hubitat's
// same-handler overwrite behavior and duplicate runOnce jobs. App-level
// singleThreaded execution makes state.schedulesJson read/modify/write atomic
// across scheduled handlers, location events, and dashboard endpoints.
// ===========================================================================

def withSchedulesLock(Closure body) {
    // Kept as a small transaction boundary for scheduler call sites. Hubitat's
    // definition(singleThreaded: true) provides the actual serialization.
    return body()
}
...
def shutdownScheduler() {
    ...
    // scheduledJobHandler is the pre-0.4.8 per-schedule handler; clear any
    // legacy jobs during upgrade before arming the single dispatcher.
    try { unschedule("scheduledJobHandler") } catch (e) {}
    try { unschedule("schedulerNextFireHandler") } catch (e) {}
    ...
    try { unsubscribe("schedulerSunEvent") } catch (e) {}
...
def initializeScheduler() {
    ...
    try { subscribe(location, "sunrise", schedulerSunEvent) } catch (e) {}
    try { subscribe(location, "sunset", schedulerSunEvent) } catch (e) {}
...
def schedulerSunTimeChanged(evt) {
    if (!schedulerIsEnabled()) return
    try { rebuildScheduledJobs() } catch (e) {
        log.warn "Modern Dashboard: sun time re-arm failed: ${e}"
    }
}

def schedulerSunEvent(evt) {
    if (!schedulerIsEnabled()) return
    def which = evt?.name?.toString()?.trim()?.toLowerCase()
    if (which != "sunrise" && which != "sunset") return
    def eventMs = now()
    def tz = location.timeZone
    def cal = Calendar.getInstance()
    if (tz) cal.setTimeZone(tz)
    def map = parseSchedulesMap()
    def ids = []
    try { ids = map.keySet().toList() } catch (e) {}
    for (id in ids) {
        def s = map[id]
        ...
        if (scheduleTriggerWhen(tr) != which) continue
        if (!scheduleDayMatchesWeekly(tr, cal)) continue
        // The location event is an independent fallback for exact sun-time
        // schedules. Offset schedules stay on the single dispatcher.
        if (scheduleOffsetMin(tr) != 0) continue
        // The dispatcher may already have consumed this occurrence. Only run
        // the event fallback while the stored occurrence is due, never early.
        def storedFire = null
        try { storedFire = s?.nextFire as Long } catch (e) {}
        if (storedFire == null || storedFire > eventMs ||
            storedFire < eventMs - schedulerDueGraceMs()) continue
        try {
            runScheduledJobById(id.toString(), false)
        } catch (e) {
            log.warn "Modern Dashboard: ${which} schedule ${id} failed: ${e}"
        }
    }
    rebuildScheduledJobs()
}
...
def schedulerMidnightRearm() {
    if (!schedulerIsEnabled()) return
    try { rebuildScheduledJobs() } catch (e) {
        log.warn "Modern Dashboard: midnight scheduler re-arm failed: ${e}"
    }
}
...
def schedulerDueGraceMs() {
    // The five-minute watchdog can recover a missed dispatcher without running
    // an old daily automation hours later.
    return 10L * 60L * 1000L
}

def schedulerExistingDueMs(s, long fromMs) {
    def nf = s?.nextFire
    if (nf == null) return null
    try {
        long t = nf as Long
        if (t >= fromMs - schedulerDueGraceMs() && t <= fromMs + 2000L) return t
    } catch (e) {}
    return null
}

def schedulerNextFireForSchedule(s, long fromMs) {
    if (!s || s?.enabled != true) return null
    def due = schedulerExistingDueMs(s, fromMs)
    ...
    if (kind == "daily" || kind == "weekly") {
        ...
        if (when == "clock") {
            ...
            return cron ? cronNextFire(cron, fromMs) : null
        }
        return scheduleSunNextFire(tr, when, fromMs)
    }
    if (kind == "once") {
        def atMs = scheduleOnceToMs(tr?.at)
        if (atMs != null && atMs >= fromMs - schedulerDueGraceMs()) return atMs
    }
    return null
}

def schedulerArmNextDispatcher(map) {
    try { unschedule("schedulerNextFireHandler") } catch (e) {}
    def earliest = null
    for (id in map.keySet()) {
        def s = map[id]
        if (!s || s?.enabled != true || s?.nextFire == null) continue
        try {
            long t = s.nextFire as Long
            if (earliest == null || t < earliest) earliest = t
        } catch (e) {}
    }
    if (earliest == null) return false
    long fireMs = Math.max(earliest as Long, now() + 1000L)
    runOnce(new Date(fireMs), "schedulerNextFireHandler", [overwrite: true])
    return true
}
...
def rebuildScheduledJobs() {
    ...
    return withSchedulesLock {
        ...
        for (id in map.keySet().toList()) {
            ...
            def nextFire = null
            try {
                nextFire = schedulerNextFireForSchedule(s, fromMs)
                if (s?.enabled == true && kind == "once" && nextFire == null) {
                    def atMs = scheduleOnceToMs(tr?.at)
                    if (atMs != null && atMs < fromMs - schedulerDueGraceMs()) removeIds << id
                    else result.failures[id.toString()] = "invalid one-time date"
                } else if (s?.enabled == true && kind != "mode" && nextFire == null) {
                    result.failures[id.toString()] = "could not compute next run time"
                }
            } catch (e) {
                log.warn "Modern Dashboard: schedule ${id} next-run failed: ${e}"
                result.failures[id.toString()] = (e?.message ?: e?.toString() ?: "next-run failed").toString()
            }
        ...
        try {
            if (schedulerArmNextDispatcher(map)) result.registered = 1
        } catch (e) {
            log.warn "Modern Dashboard: dispatcher arm failed: ${e}"
            for (id in map.keySet()) {
                def s = map[id]
                if (s?.enabled == true && s?.nextFire != null) {
                    result.failures[id.toString()] = (e?.message ?: e?.toString() ?: "dispatcher arm failed").toString()
                }
            }
        }
        logDbg("scheduler rebuild — dispatcher=${result.registered == 1}")
        return result
    }
}

def schedulerDueScheduleIds(map, long atMs) {
    def due = []
    // Never consume a future occurrence. If Hubitat invokes runOnce early,
    // rebuild simply re-arms the same dispatcher at least one second later.
    long cutoff = atMs
    long oldest = atMs - schedulerDueGraceMs()
    for (id in map.keySet()) {
        def s = map[id]
        if (!s || s?.enabled != true || s?.nextFire == null) continue
        try {
            long t = s.nextFire as Long
            if (t >= oldest && t <= cutoff) due << [id: id.toString(), at: t]
        } catch (e) {}
    }
    due.sort { a, b ->
        def byTime = (a.at as Long) <=> (b.at as Long)
        return byTime != 0 ? byTime : (a.id <=> b.id)
    }
    return due.collect { it.id }
}

def schedulerNextFireHandler() {
    if (!schedulerIsEnabled()) return
    def map = parseSchedulesMap()
    def ids = schedulerDueScheduleIds(map, now())
    for (id in ids) {
        try {
            runScheduledJobById(id.toString(), false)
        } catch (e) {
            log.warn "Modern Dashboard: schedule ${id} failed: ${e}"
        }
    }
    rebuildScheduledJobs()
}

// Compatibility path for jobs left by an older app version during upgrade.
def scheduledJobHandler(data) {
    if (!schedulerIsEnabled()) return
    schedulerNextFireHandler()
}
...
// Advance nextFire after a trigger is consumed (whether action ran or mode-skipped).
// The single dispatcher is re-armed after the due batch completes.
def scheduleAdvanceAfterTrigger(id, s, map, boolean markFired) {
    ...
    try {
        if (kind == "daily" || kind == "weekly") {
            // Clear the consumed occurrence so the due-preservation guard does
            // not return it when computing the next one.
            s.nextFire = null
            s.nextFire = schedulerNextFireForSchedule(s, now())
            saveSchedulesMap(map)
            return false
        }
    } catch (e) {
...
def runScheduledJobById(id, boolean rebuildAfter = true) {
    if (!id) return
    def action = null
    def logSource = null
    def advanced = false
    def shouldRun = withSchedulesLock {
        def map = parseSchedulesMap()
        def s = map[id]
        if (!s) return false
        ...
        if (kind != "mode") {
            ...
            if (onlyInModes instanceof List && onlyInModes) {
                ...
                if (!ok) {
                    ...
                    // Trigger consumed without running — still advance nextFire.
        ...
        action = s?.action
        logSource = "automation ${scheduleLogName(id, s)}"
        // Persist the consumed occurrence before issuing device commands.
    ...
    if (shouldRun == true) {
        try {
            runScheduleAction(action, logSource)
        } catch (e) {
            log.warn "Modern Dashboard: schedule ${id} action failed: ${e}"
        }
    }
    if (rebuildAfter && advanced) rebuildScheduledJobs()
}
...
def runScheduleAction(action, logSource) {
    if (!action) return
    if (!logSource) logSource = "automation"
    def target = action?.target?.toString()
    switch (target) {
        case "lights":
            runScheduleLightAction(action, logSource)
            break
        case "outlets":
            runScheduleOnOffAction(action, outletSwitches, logSource)
            break
        case "thermostats":
            runScheduleThermostatAction(action, logSource)
            break
        case "hubMode":
            def mode = action?.mode?.toString()?.trim()
            if (mode) {
                try {
                    logControl(logSource, "hub mode → ${mode}")
...
def runScheduleOnOffAction(action, deviceList, logSource) {
    ...
    for (st in states) {
        ...
        try {
            ...
            logControl(logSource, "${deviceLabel(dev)} ${cmd}")
...
def runScheduleLightAction(action, logSource) {
    def states = action?.states
    if (!(states instanceof List)) return
    for (st in states) {
        def id = st?.id
        ...
        try {
            ...
            if (on) {
                ...
                logControl(logSource, "${deviceLabel(dev)} ${detail}")
                dev.on()
                def lvl = st?.level
                ...
                logControl(logSource, "${deviceLabel(dev)} off")
                dev.off()
            }
        } catch (e) {
...
def runScheduleThermostatAction(action, logSource) {
    ...
    for (id in ids) {
        ...
        try {
            ...
            if (bits) logControl(logSource, "${deviceLabel(dev)} ${bits.join(' ')}")
            if (mode) {
...
def cleanupSchedules() {
    if (!schedulerIsEnabled()) return
    // Watchdog: recover a dispatcher callback delayed or lost by the hub.
    // Due one-time schedules run before they are removed.
    schedulerNextFireHandler()
}
...
def schedulesValidateNormalized(s) {
    ...
    if (!(target in ["lights", "outlets", "thermostats", "hubMode"])) return "unsupported action type"
...
def schedulesSave() {
    ...
    if (validationError) {
        return renderJsonNoStore( "{\"ok\":false,\"error\":${jsonStr(validationError)}}".toString(), 422)
    }
    def priorJson = null
    def id = withSchedulesLock {
        def map = parseSchedulesMap()
        priorJson = state.schedulesJson
        def sid = s.id
        if (!sid) sid = scheduleNewId()
        def existing = map[sid]
...
def schedulesToggle() {
    ...
    def current = parseSchedulesMap()[id]
    if (!current) {
        return renderJsonNoStore( '{"ok":false,"error":"not found"}', 404)
    }
    if (current?.enabled != true) {
        def validationError = schedulesValidateNormalized(current)
        if (validationError) {
            return renderJsonNoStore( "{\"ok\":false,\"error\":${jsonStr(validationError)}}".toString(), 422)
        }
    }
    def priorJson = null
    def enabledNow = withSchedulesLock {
        def map = parseSchedulesMap()
        def row = map[id]
        if (!row) return null
        priorJson = state.schedulesJson
    ...
    def out = new StringBuilder()
    out << "{\"ok\":true,\"id\":" << jsonStr(id.toString())
    out << ",\"enabled\":" << (enabledNow == true ? "true" : "false")
    out << ",\"schedules\":" << groovy.json.JsonOutput.toJson(schedulesListForClient())
    out << "}"
    return renderJsonNoStore( withAuthJson(out.toString()), 200)
}
...
def schedulesTest() {
    ...
    try {
        ...
        runScheduleAction(s?.action, "automation ${scheduleLogName(id, s)}")
    } catch (e) {
